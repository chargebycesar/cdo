...
import { CloudDiagnostics } from './CloudDiagnostics';
import type { InformeNube } from '../lib/cloudSync';
...
interface Props {
  ...
  onInspeccionarNube?: () => Promise<InformeNube>;
  onTraerNube?: () => Promise<void>;
  onSubirNube?: () => Promise<void>;
}

export const SettingsView: React.FC<Props> = ({ companySettings, onSaveSettings, onDeleteExamples, onCargarEjemplos, hayDemo, estadoCompleto, onRestaurarEstado, firebaseUser, estadoNube, errorNube, onAviso, onInspeccionarNube, onTraerNube, onSubirNube }) => {
      ...
      <form onSubmit={guardar} className="space-y-6">
        ...
        <Seccion icono={<Cloud size={22} />} color="indigo" titulo="Cuenta de Google, nube y Google Calendar" sub="Opcional. Con la cuenta vinculada, los datos se guardan en la nube y se sincronizan en todos tus dispositivos en tiempo real.">
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 space-y-3 text-xs">
            ...
            {onInspeccionarNube && onTraerNube && onSubirNube && <CloudDiagnostics firebaseUser={firebaseUser} estadoCompleto={estadoCompleto} estadoNube={estadoNube} errorNube={errorNube} onInspeccionar={onInspeccionarNube} onTraer={onTraerNube} onSubir={onSubirNube} onAviso={onAviso} />}
          </div>
        </Seccion>

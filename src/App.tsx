...
import { guardarEstadoEnNube, cargarEstadoDeNube, escucharEstadoNube, inspeccionarNube, traerEstadoDelServidor } from './lib/cloudSync';
...
function AppPrincipal() {
  ...
  // ---- Diagnóstico de la nube: mirar, traer y subir a mano ----
  const inspeccionarNubeAhora = async () => {
    if (!firebaseUser) throw new Error('No hay cuenta de Google vinculada.');
    return inspeccionarNube(firebaseUser.uid);
  };
  const traerNubeAhora = async () => {
    if (!firebaseUser) throw new Error('No hay cuenta de Google vinculada.');
    const remoto = await traerEstadoDelServidor(firebaseUser.uid);
    if (!remoto) throw new Error('En la nube de esta cuenta no hay ningún estado guardado.');
    guardarCopiaAnterior(estadoCompleto);
    ultimoRemotoAplicado.current = remoto.updatedAt;
    aplicarEstado({ ...remoto, syncUid: firebaseUser.uid });
    setSyncUid(firebaseUser.uid);
    guardarMarcaNube(remoto.updatedAt);
    setEstadoNube('sincronizado');
    setErrorNube(null);
  };
  const subirNubeAhora = async () => {
    if (!firebaseUser) throw new Error('No hay cuenta de Google vinculada.');
    const st = { ...estadoCompleto, syncUid: firebaseUser.uid, updatedAt: new Date().toISOString() };
    ultimoRemotoAplicado.current = st.updatedAt;
    await guardarEstadoEnNube(firebaseUser.uid, st);
    setSyncUid(firebaseUser.uid);
    guardarMarcaNube(st.updatedAt);
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-slate-50">
        ...
        {(() => {
          ...
          if (firebaseUser && estadoNube === 'error' && activeTab !== 'ajustes') {
            return (
              <div className="mx-4 lg:mx-8 mt-3 p-3 rounded-2xl border border-rose-200 bg-rose-50 text-xs font-bold text-rose-900 flex items-center gap-3">
                <CloudOff size={15} className="shrink-0" />
                <span className="flex-1">No se está guardando en la nube{errorNube ? `: ${errorNube}` : ''}. Lo que cambies aquí no llegará a tus otros dispositivos hasta que se arregle.</span>
                <button onClick={() => irA('ajustes')} className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-black cursor-pointer shrink-0">Ver qué pasa</button>
              </div>
            );
          }
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50 relative">
          ...
          {activeTab === 'ajustes' && (
            <SettingsView companySettings={companySettings} onSaveSettings={setCompanySettings} onDeleteExamples={() => setShowDeleteDemoModal(true)} onCargarEjemplos={handleCargarEjemplos} hayDemo={hayDemo}
              ...
              onInspeccionarNube={inspeccionarNubeAhora} onTraerNube={traerNubeAhora} onSubirNube={subirNubeAhora} />
          )}
        </main>
      </div>

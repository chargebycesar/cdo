...
import { auth, db, googleProvider, signInWithPopup, signOut, doc, setDoc, getDoc, getDocFromServer, onSnapshot, User } from './firebase';
...
// ---- Diagnóstico ----
// Lo que hay en la nube, leído directamente del servidor (sin pasar por la caché local, que
// es justo lo que confunde cuando algo no cuadra): el estado principal y la última foto manual.
export interface InformeNube {
  principal: AppState | null;
  kb: number;
  copiaManual: AppState | null;
}

export const inspeccionarNube = async (userId: string): Promise<InformeNube> => {
  const snap = await getDocFromServer(refPrincipal(userId));
  const principal = snap.exists() ? migrarEstado(snap.data()) : null;
  const kb = snap.exists() ? Math.round(JSON.stringify(snap.data()).length / 1024) : 0;
  let copiaManual: AppState | null = null;
  try {
    const c = await getDocFromServer(refCopia(userId, 'ultima'));
    if (c.exists()) copiaManual = migrarEstado(c.data());
  } catch {
    // sin foto manual
  }
  return { principal, kb, copiaManual };
};

// El estado de la nube tal cual está en el servidor ahora mismo.
export const traerEstadoDelServidor = async (userId: string): Promise<AppState | null> => {
  const snap = await getDocFromServer(refPrincipal(userId));
  return snap.exists() ? migrarEstado(snap.data()) : null;
};
